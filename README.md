...
```
tests/b2bTurkeyBron.spec.cjs — Главный сценарий теста для Playwright Test Runner
playwright.config.js  — Конфигурация Playwright (браузер, таймауты, baseURL и т.д.)
loginPage.cjs          — Page Object: авторизация на сайте
searchPage.cjs         — Page Object: поиск тура
bookingPage.cjs        — Page Object: бронирование (туристы + покупатель)
notify.cjs             — Утилита: отправка уведомлений в Mattermost
...
## 4. `tests/b2bTurkeyBron.spec.cjs` — Главный сценарий

**Назначение:** Выполняет шаги теста последовательно через Playwright Test Runner.

1. **Авторизация** — открыть страницу, нажать "Вход", ввести логин/пароль
2. **Поиск тура** — выбрать Москва → Турция → дата 05.10.2026 → снять группировку → включить "не показывать PROMO" → Поиск
3. **Выбор тура** — клик по первой цене, ожидание открытия новой вкладки (или той же страницы)
4. **Заполнение данных** — турист 1, турист 2, отметить заказчика, заполнить покупателя
5. **Пересчёт и бронирование** — нажать "Пересчитать", затем "бронировать"

**Обработка ошибок:** При любой ошибке отправляется уведомление в Mattermost с указанием шага (`currentStep`) и URL страницы.

---

## 5. `notify.cjs` — Утилита уведомлений
